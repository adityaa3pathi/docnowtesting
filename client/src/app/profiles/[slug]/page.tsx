...
export default function ProfileDetailsPage(props: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      try {
        const { dealTypeId, dealType, partnerCode } = parseSlug(params.slug, 'profiles');
        if (!dealTypeId || !partnerCode) throw new Error('Invalid profile URL');
        ...
        const richDealType = dealType || 'profile';
        ...
        try {
          ...
          console.warn('[Profile Details] Failed to fetch rich data:', richErr);
        }

      } catch (err: any) {
        console.error('[Profile Details] Error:', err);
        setError('We encountered an unexpected issue while loading the profile details. Please try again later.');
      } finally {
        setLoading(false);
      }
    };
      ...
      <main className="min-h-screen flex flex-col bg-gray-50">
        <div className="flex-1 flex flex-col items-center justify-center p-4">
          ...
          <p className="text-gray-500 mb-6 text-center max-w-md">
            {error || 'The profile you are looking for does not exist or is currently unavailable.'}
          </p>
          <Button onClick={() => router.push('/search?type=PROFILE')}>View All Profiles</Button>
        </div>
        <Footer />
      </main>
    );
  ...
  return (
    <main className="flex flex-col min-h-screen bg-gray-50 pb-24">
      ...
      {/* Hero Section */}
      <section className="bg-gradient-to-br from-teal-900 via-teal-800 to-emerald-900 pt-8 pb-16">
        <div className="container mx-auto px-4 max-w-4xl">
          <button
            onClick={() => router.push('/search?type=PROFILE')}
      ...
      <section className="container mx-auto px-4 max-w-4xl -mt-6 relative z-10">
        <div className="grid gap-6">
          ...
          {/* Description Card */}
          {hasValidDescription && (
            <Card className="p-6 md:p-8 border-gray-100 shadow-xl shadow-teal-900/5">
              ...
              {showConstituents && (
                <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-3 pt-6 border-t border-gray-100">
                  {richData.constituents.map((c, idx) => (
                    <div key={c.id} className="flex items-start gap-3 p-3 rounded-xl hover:bg-teal-50 transition-colors border border-transparent hover:border-teal-100 group">
                      <div className="flex items-center justify-center w-6 h-6 rounded-full bg-teal-100 text-teal-700 text-xs font-bold shrink-0 mt-0.5">
                        {idx + 1}
                      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] z-50">
        <div className="container mx-auto max-w-4xl flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Button
              size="lg"
              className={`px-8 md:px-12 transition-all ${inCart ? 'bg-slate-900 hover:bg-slate-800 text-white' : 'bg-teal-600 hover:bg-teal-700'}`}
