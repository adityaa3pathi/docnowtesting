...
async function main() {
    ...
    const adminEmail = process.env.ADMIN_EMAIL || 'admin@docnow.in';
    const rawPassword = process.env.ADMIN_PASSWORD || 'DocNowSecure#2026';
    
    if (!process.env.ADMIN_PASSWORD) {
        console.warn('⚠️  ADMIN_PASSWORD is not set in environment. Using default initial password.');
    }
    ...
    if (!existingAdmin) {
        const admin = await prisma.user.upsert({
            ...
            create: {
                ...
                status: 'ACTIVE',
                isVerified: true,
                referralCode: `ADMIN${nanoid(4).toUpperCase()}`,
            },
        });
    ...
    for (const user of usersWithoutCode) {
        ...
        const code = `${prefix}${nanoid(5).toUpperCase()}`;
