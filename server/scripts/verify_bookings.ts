...
async function verifyBookingFlow() {
    try {
        ...
        console.log('\n--- 2. Manage Address ---');
        // List addresses
        const addrRes = await axios.get(`${API_URL}/profile/addresses`, authHeaders);
        let addressId = '';
        if (addrRes.data.length === 0) {
            console.log('Creating new address...');
            const newAddr = await axios.post(`${API_URL}/profile/addresses`, {
        ...
        console.log('\n--- 3. Add to Cart ---');
        // Clear cart first
        await axios.delete(`${API_URL}/cart`, authHeaders).catch(() => { });
        ...
        const freezeRes = await axios.post(`${API_URL}/slots/freeze`, {
        ...
        // 6. List Bookings
        console.log('\n--- 6. List Bookings ---');
        const listRes = await axios.get(`${API_URL}/bookings`, authHeaders);
        console.log(`Found ${listRes.data.length} bookings.`);
        const found = listRes.data.find((b: any) => b.id === newBookingId);
        if (!found) throw new Error('New booking not found in list!');
        console.log('Verified booking is in list.');

        // 7. Track Status
        console.log('\n--- 7. Track Status ---');
        const statusRes = await axios.get(`${API_URL}/bookings/${newBookingId}/status`, authHeaders);
