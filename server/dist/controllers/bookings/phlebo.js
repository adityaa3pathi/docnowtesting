...
exports.getPhleboContact = getPhleboContact;
 ...
 */
async function getPhleboContact(req, res) {
    try {
        ...
        // 3. Ownership Check
        const booking = await booking_service_1.BookingService.getBookingWithAuth(bookingId, userId);
        if (!booking) {
        ...
        const { bookingStatus } = await booking_service_1.BookingService.getHealthiansCustomers(booking.partnerBookingId);
