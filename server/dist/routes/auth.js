...
router.post('/signup/verify', async (req, res) => {
    try {
        ...
        // Issue Token
        const token = jsonwebtoken_1.default.sign({ userId: newUser.id, mobile: newUser.mobile }, JWT_SECRET, { expiresIn: '7d' });
        res.status(201).json({
...
router.post('/login/password', async (req, res) => {
    try {
        ...
        const token = jsonwebtoken_1.default.sign({ userId: user.id, mobile: user.mobile }, JWT_SECRET, { expiresIn: '7d' });
        res.status(200).json({
